...
from src.vertex import FullVertex
from src.dag import DAG


def main() -> None:
    # feature_1: LLMFeature = LLMFeature(
    #     name="something", prompt_template="What is {a} + {b}? 1"
    # )
    # feature_2: LLMFeature = LLMFeature(
    #     name="something", prompt_template="What is {a} + {b}? 0", priority=0
    # )
    # feature_3: LLMFeature = LLMFeature(
    #     name="something", prompt_template="What is {a} + {b}? 3", priority=3
    # )
    # llm_node: LLMNode = LLMNode(
    #     features=[feature_1, feature_2, feature_3], model=None
    # )
    ...
    condition_vertex = FullVertex(condition_function=lambda x: True)
    execute_vertex = FullVertex(execute_function=lambda x: x | {"vertex": True})
    full_vertex = FullVertex(
        condition_function=lambda x: True,
        execute_function=lambda x: x | {"full_vertex": True},
    )
